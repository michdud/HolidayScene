Shader.source[document.currentScript.src.split('js/shaders/')[1]] = `#version 300 es 
  ...
  void main(void) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      if (!wasHit) {
        fragmentColor += texture(material.envTexture, d.xyz ) * vec4(wNew, 1.0);
        wNew = vec3(0, 0, 0);
      }
      ...
      if (wasHit) {
        ...
        vec3 color = clippedQuadrics[bestIndex].materialColor;
        ...
        for (int i = 0; i < lights.length(); i++) {
          ...
          if (!shadowRayHitSomething || bestShadowT * lights[i].position.w > sqrt(dot(lightDiff, lightDiff))) {
            ...
            fragmentColor.rgb += shade(normal, lightDir, -d.xyz, powerDensity, color, clippedQuadrics[bestIndex].specularColor, clippedQuadrics[bestIndex].shininess) * wNew;
          }
          
        }
        e = hit + vec4(normal * 0.01, 0.0);
        d = vec4(reflect(d.xyz, normal), 0.0);
        wNew *= clippedQuadrics[bestIndex].reflectance;
        
      }
      if (dot(wNew, wNew) < 0.1) {
          break;
      }
